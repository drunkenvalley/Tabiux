...
tabiux.height = 36
tabiux.width = 200
tabiux.curve = CurveConstants.ScaleTo100
...
tabiux.statusbar:SetValue(UnitHealthPercent(tabiux.target, false, tabiux.curve))
...
tabiux.text:SetText(UnitHealthPercent(tabiux.target, false, tabiux.curve) .. "%")
    ...
    if event and event == "UNIT_HEALTH" then
        tabiux.text:SetText(
            C_StringUtil.RoundToNearestString(
                UnitHealthPercent(tabiux.target, false, tabiux.curve)
            ) .. "%"
        )
        tabiux.statusbar:SetValue(UnitHealthPercent(tabiux.target, false, tabiux.curve))
    end
